                               ...
                               const std::vector<std::vector<unsigned int>>& LIMB_COLORS)
{
    ...
    for (auto& obj : objs) {
        cv::rectangle(image, obj.rect, {0, 0, 255}, 2);

        char text[256];
        sprintf(text, "athlete %.1f%%", obj.prob * 100);
        ...
        for (int k = 0; k < num_point + 2; k++) {
            if (k < num_point) {
                ...
                // cv::putText(image, std::to_string(k), cv::Point(kps_x, kps_y + 5), cv::FONT_HERSHEY_SIMPLEX, 1, {255, 0, 0}, 1);
            }
            auto& ske    = SKELETON[k];
